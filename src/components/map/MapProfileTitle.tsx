...
//
// It is the page's H1 (DesktopProfileModal drops to h2 while this is mounted, so
// the one-h1-per-page rule still holds). That is deliberate: the crawler-visible
// heading on a bare /p/<id> comes from the SPA - the worker only injects <head>
// there - so a heading carrying "<opinio name> + what the world thinks" is worth
// more than the bare name the modal used to supply.
//
...
export function MapProfileTitle({ profile, hasVotes }: { profile: Profile | null; hasVotes: boolean }) {
  ...
  return (
    ...
    >
      {profile && (
        <h1 className="flex w-full flex-col items-center gap-1.5 rounded-xl bg-surface-light/60 backdrop-blur-md ring-1 ring-white/[0.08] shadow-xl shadow-black/20 px-6 py-4">
          ...
          <span className="flex items-center justify-center gap-3 w-full min-w-0">
            <Avatar
              ...
              className="w-11 h-11 shrink-0 text-sm ring-2 ring-white/10"
            />
